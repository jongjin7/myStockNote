...
function RecentMemoItem({ memo, stock }: RecentMemoItemProps) {
 ...
 <Link to={`/stocks/${stock?.id}`}>
  ...
  <div className="flex items-center text-sm font-black text-gray-600 group-hover:text-primary-400 transition-all gap-2 uppercase tracking-[0.2em]">
...
export function RecentMemosSection({ recentMemos, stocks }: RecentMemosSectionProps) {
 ...
 <section className="lg:col-span-2 space-y-8">
  ...
  {recentMemos.length > 0 ? (
   ...
   <p className="text-gray-700 text-[11px] font-black uppercase tracking-[0.3em] ">기록 대기 중...</p>
   </div>
  )}
  </div>
 </section>
