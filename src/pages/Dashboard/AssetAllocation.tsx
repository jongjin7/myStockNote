    ...
    <div className="bg-gray-900/40 backdrop-blur-sm border border-gray-800/50 rounded-3xl p-8 h-full flex flex-col relative overflow-hidden group/card">
      ...
      <div className="flex-1 flex flex-col xl:flex-row items-center justify-center gap-12 py-4 relative z-10">
        ...
        <div className="relative w-72 h-72 shrink-0 flex items-center justify-center">
          ...
          >
            {/* Render inactive or neutral slices first, then the active slice to keep it on top */}
            {[...displayStocks]
              ...
              .map((stock) => {
                ...
                // Find original index for color tracking
                const originalIndex = displayStocks.findIndex(s => s.id === stock.id);
                
                // Calculate offset based on original order to maintain correct pie positions
                let currentOffset = 0;
          ...
          {sortedAll.length > 0 && (
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-4 bg-gray-950/90 backdrop-blur-xl border border-white/10 px-4 py-1.5 rounded-full text-[10px] font-black text-white shadow-[0_20px_40px_rgba(0,0,0,0.4)] z-20 flex items-center gap-2 whitespace-nowrap group-hover/card:border-primary-500/30 transition-all">
              ...
              <span className="text-white">
                {activeStockId 
                  ? displayStocks.find(s => s.id === activeStockId)?.name 
                  : sortedAll[0].name}
              </span>
              <span className="text-gray-500 mx-1">|</span>
              <span className="text-white tabular-nums">
                {activeStockId 
                  ? `${(displayStocks.find(s => s.id === activeStockId) ? ((displayStocks.find(s => s.id === activeStockId)!.currentPrice || displayStocks.find(s => s.id === activeStockId)!.avgPrice) * displayStocks.find(s => s.id === activeStockId)!.quantity / totalEvaluation * 100) : 0).toFixed(1)}%`
                  : `${topWeight.toFixed(1)}%`}
              </span>
            </div>
          )}
        ...
        <div className="flex-1 w-full max-w-md flex flex-col justify-between h-full space-y-6">
          <div className="grid grid-cols-1 gap-3">
            {displayStocks.map((stock, index) => {
              ...
              return (
                <div 
                  key={stock.id} 
                  onClick={() => setActiveStockId(isActive ? null : stock.id)}
                  className={cn(
                ...
                >
                  ...
                  <div className="text-right flex flex-col items-end">
                    ...
                    <div className="w-16 h-1 bg-gray-800/30 rounded-full mt-1.5 overflow-hidden">
                      <div 
                        className={cn("h-full transition-all duration-1000 delay-500 opacity-60", bgColors[index])} 
          ...
          {/* Bottom Insights Bar (Fluid Design Synchronized with Image) */}
          <div className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-5 flex items-center justify-between divide-x divide-white/10 mt-auto shadow-xl">
            ...
            <div className="flex flex-col items-center justify-center flex-1 px-2">
              ...
              <span className="text-sm font-black text-white tabular-nums tracking-tighter">{sortedAll.length}개</span>
            </div>
          </div>
        </div>
      </div>
